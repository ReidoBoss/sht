file://<WORKSPACE>/app/model/services/HabitServices.scala
...
action parameters:
offset: 1300
uri: file://<WORKSPACE>/app/model/services/HabitServices.scala
text:
```scala
...
class HabitServices @Inject()(
  ...
  def getTodaysHabitById(id:UUID) = {
    for {
      idNow <- historyRepo.getById(id)
      realResult <- idNow match {
        case Some(id) => historyRepo.updateById(@@)
      }
      result <- Future("meow")
    } yield result
  }
